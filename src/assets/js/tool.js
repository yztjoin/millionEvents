export default Tool = {
  ...
  */
  ...
  initDate(MAXTIME, MINTIME, nowText) {
    ...
    days = (nowHour + Math.floor(leftSeconds / 3600) >= 24 && days == 1) ?
      (days + (Math.ceil((Math.floor(leftSeconds / 3600) - (24 - nowHour)) / 24))) :
      days + 1;
    for (let i = 1; i <= days; i++) {
      let curDate = new Date(Date.parse(now) + ((i - 1 + (i == 1 ? 0 : minDay)) * 60 * 60 * 24 * 1000));
      let that = this;
      arr[i - 1] = {
        ...
        children: function () {
          ...
          for (let m1 = 0, m = (i == 1) ? ((minLeftMin + roundMinutes) >= 60 ? ((nowHour + minLength) + 1) : (nowHour + minLength)) : ((i == 2 && minDay >= 1) ? (minLength) : (i == 2 && minLength + nowHour > 24 ? Math.abs(24 - (minLength + nowHour)) : 0)); m < hourEnd; m1++, m++) {
            minutes[m1] = {
              label: m + '时',
              value: m,
              children: function () {
                let seconds = [];
                for (let s1 = 0, s = (s1 == 0 && m1 == 0 && i == 1 ? (dealWithMin(nowMinutes) > 50 ? 0 : dealWithMin(nowMinutes)) : 0); s < 60; s += INTERVAL, s1++) {
                  seconds[s1] = {
                    label: s + '分',
                    value: s
                  }
